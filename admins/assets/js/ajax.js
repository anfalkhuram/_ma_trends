...
$(document).on('click', '#status-update', function () {
    ...
    $.ajax({
        ...
        error: function () {
            ...
            setTimeout(function () {
                btn.prop('disabled', false)
                    .removeClass('badge-trending badge-ma')
                    .css({
...
// edit sub category
$('#editSubCategory').on('hidden.bs.modal', function () {
    location.reload();
});
...
$(document).on('click', '.edit-btn', function () {
    let btn = $(this);
    const id = btn.data('id');
    const category_id = btn.data('categoryid');
    const name = btn.data('name');
    const status = btn.data('status');
    ...
    // $('select[name="category_id"]').val(String(category_id));
