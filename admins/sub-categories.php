...
if (isset($_POST["updateStatus"])) {
    ...
    // $sql = "UPDATE sub_categories SET status = $new_status WHERE id = $id";
    $sql = "UPDATE sub_categories SET status = $new_status WHERE id = $id AND category_id IN ( SELECT id FROM categories WHERE status = 1)";
...
if (isset($_GET['action']) && $_GET['action'] == 'delete') {
    ...
    $id = $_GET['id'];
    $resultCategory = mysqli_query($conn, "DELETE FROM sub_categories WHERE id = $id");
    if ($resultCategory) {
...
if (isset($_POST['add_subCategory'])) {
    ...
    $sql = "INSERT INTO sub_categories (category_id, name, status) VALUES ('$category', '$name', '$status')";
    $result = mysqli_query($conn, $sql);
    if ($result) {
        echo 1;
    } else {
...
if (isset($_POST['edit_subCategory'])) {
    ...
    $category = (int)$_POST['category_id'];
    $status = (int)$_POST['status'];
    $sql = "UPDATE sub_categories SET category_id = '$category', name = '$name', status = '$status' WHERE id = '$id'";
    $result = mysqli_query($conn, $sql);
    if ($result) {
        echo 1;
    } else {
...
<body>
    <div class="ma-admin-shell d-flex flex-column flex-md-row">
        ...
        <main class="flex-grow-1">
            ...
            <div class="d-flex justify-content-end">
                <button class="btn btn-ma mt-3 me-4" data-bs-toggle="modal" data-bs-target="#addSubCategory">Add Sub Category</button>
            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-admin align-middle mb-0" id="myTable">
                            ...
                            <tbody class="js-admin-tbody">
                                <?php
                                $sqlCategories = "SELECT * FROM sub_categories";
                                ...
                                ?>
                                        <tr class="js-admin-row" data-sr="<?php echo $sr; ?>" data-id="<?php echo $row["id"]; ?>" data-name="<?php echo $row["name"]; ?>" data-status="<?php echo $row["status"]; ?>">
                                            ...
                                            <td class="btn-status">
                                                ...
                                                <button type="button" class="btn btn-sm btn-ma-outline <?php echo $btnClass; ?> rounded-pill fw-bold px-2 py-0 small text-capitalize" data-id="<?php echo $row["id"]; ?>" data-status="<?php echo $row["status"]; ?>" id="status-update" <?php echo ($row_category['status'] == 1) ? '' : 'disabled'; ?>>
                                            ...
                                            <td class="text-end">
                                                <!-- edit button -->
                                                <button class="btn btn-sm btn-ma-outline edit-btn" data-bs-toggle="modal" data-bs-target="#editSubCategory"
                                                    data-id="<?php echo $row["id"]; ?>" data-categoryid="<?php echo $row["category_id"]; ?>"
                                                    data-name="<?php echo $row["name"]; ?>" data-status="<?php echo $row["status"]; ?>">Edit</button>
                                                <!-- delete button -->
                                                <button
    ...
    <div class="modal fade" id="addSubCategory" tabindex="-1" aria-hidden="true">
        <form class="modal-dialog modal-dialog-centered" method="post" action="sub-categories" id="add_subCategory_form">
            <div class="modal-content ma-bg-surface border ma-border ma-rounded">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input class="form-control" placeholder="Sub Category name" name="name" required />
                    </div>
                    <div class="mb-3">
                ...
                <div class="modal-footer border-0">
                    ...
                    <button class="btn btn-ma" type="submit" id="add_subCategory_btn">Save</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <div class="modal fade" id="editSubCategory" tabindex="-1" aria-hidden="true">
        <form class="modal-dialog modal-dialog-centered" action="sub-categories" method="post" id="edit_subCategory_form">
            <div class="modal-content ma-bg-surface border ma-border ma-rounded">
                <div class="modal-header border-0">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label ma-muted">Name</label>
                        <input class="form-control" placeholder="Category name" name="name" value="<?php echo $subCategory['name']; ?>" required />
                    </div>
                    <div class="mb-3">
                        <label class="form-label ma-muted">Category</label>
                        <select class="form-select" name="category_id" required>
                ...
                <div class="modal-footer border-0">
                    ...
                    <button class="btn btn-ma" type="submit" id="edit_subCategory_btn">Save</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content ma-bg-surface border ma-border ma-rounded">
                ...
                <div class="modal-body ma-muted">Are you sure to delete <span class="badge badge-trending rounded-pill text-capitalize fs-6">Sub Category name</span></div>
                <div class="modal-footer border-0">
    ...
    <script src="./assets/js/sub_categories.js"></script>
</body>
